...
**Repo note:** this spec was written while `frontend_v2` was a subdirectory of the `MediGem` monorepo. It now lives in its own repo, `medigem-web`, so paths in this document that used to read `frontend_v2/...` are relative to this repo's root instead. Every reference below to `backend/...` (the Gemma inference pipeline, the emergency safety engine, `output_schema.py`, `safety.py`) points at code in the sibling `MediGem` repo, not at anything in this one. Those references are cited for schema and safety alignment; nothing in this repo edits them.

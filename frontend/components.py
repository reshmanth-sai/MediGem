        ...
        <div class="medigem-header">
            <div style="display: flex; align-items: center; justify-content: space-between;">
                ...
                <div style="background: rgba(255, 255, 255, 0.2); padding: 0.4rem 0.8rem; border-radius: 9999px; font-size: 0.8rem; font-weight: 600; text-transform: uppercase; letter-spacing: 0.05em; display: flex; align-items: center; gap: 0.4rem;">
                    <span>🟢 OFFLINE READY</span> | <span>MODEL: GEMMA 3 4B</span>
                </div>
            </div>
        </div>
        """
...
def create_demo_gallery() -> Tuple[gr.Button, gr.Button, gr.Button, gr.Button]:
    """Create one-click demo preset gallery buttons."""
    gr.Markdown("### 🚀 Demo Mode (One-Click Presets)")
    with gr.Row():
        btn_report = gr.Button("📄 Lab Report", size="sm", variant="secondary")
        btn_ecg = gr.Button("❤️ ECG Strip", size="sm", variant="secondary")
        btn_rx = gr.Button("💊 Prescription", size="sm", variant="secondary")
        btn_wound = gr.Button("🩹 Wound Inspection", size="sm", variant="secondary")
...
def create_footer() -> gr.HTML:
    ...
    return gr.HTML(
        ...
        <div class="medigem-footer">
            <p><strong>MediGem Offline AI Co-Pilot</strong> v1.0.0 | Engine: Gemma 3 4B | Host: Ollama Local</p>

...
2. **Emergency gate** (`backend/emergency`): symptoms are normalised through a synonym table and matched against `rules.json`. A match returns `EMERGENCY_INTERCEPTED` with a `RiskAssessment` and a referral; the model is never called. Median 0.20 ms (<1.0 ms max in harness, target <5.0 ms met).
